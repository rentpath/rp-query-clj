...
(deftest test-select
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/select input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/select input)))
...
(deftest test-roots-where
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/roots-where input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/roots-where input)))
...
(deftest test-text-where
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/text-where input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/text-where input)))
...
(deftest test-range-where
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/range-where input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/range-where input)))
...
(deftest test-boolean-where
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/boolean-where input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/boolean-where input)))
...
(def valid-where? (partial s/valid? ::dsl/where))
...
(deftest test-group
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/group input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/group input)))
...
(deftest test-sort
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/sort input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/sort input)))
...
(deftest test-query
  (are [expected-output input]
      (let [actual (s/valid? ::dsl/query input)]
        (is (= expected-output actual)
            (s/explain-str ::dsl/query input)))
